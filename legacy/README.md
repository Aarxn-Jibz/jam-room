...
This application relies on Next middleware and `/api/*` rewrites, so deploy the
frontend as a Cloudflare **Worker**, not as a static Cloudflare Pages export.
The Worker is the public origin; its existing `/api/*` rewrite proxies requests
to the backend Worker. Browser calls remain same-origin, so no browser CORS
configuration is required.

1. Deploy the backend Worker first and note its HTTPS `workers.dev` URL.
...
strong value used by the frontend Worker; the application no longer uses a
default production secret. Configure a Cloudflare rate-limit/WAF rule for
`/api/auth/login` before production (for example, a small per-IP limit over a
one-minute window).
