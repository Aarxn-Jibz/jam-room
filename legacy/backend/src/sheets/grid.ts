...
/**
 * Builds the deterministic weekly grid for the IST week starting at `weekMonday`.
...
export async function buildWeeklyGrid(db: DbClient, weekMonday: number): Promise<WeeklyGrid> {
  ...
  for (const booking of bookings) {
    const istDate = new Date(booking.startTime + IST_OFFSET_MS);
    const gridIndex = dayIndexWithinWeek(istDate.getUTCDay());
    const time = formatIstHhMm(booking.startTime);
