...
export const IST_OFFSET_MS = 5.5 * MS_PER_HOUR;
...
/** Monday 00:00 in Asia/Kolkata, represented as its actual UTC instant. */
export function weekMondayIst(nowMs: number): number {
  const d = new Date(nowMs + IST_OFFSET_MS);
  const daysSinceMonday = (d.getUTCDay() + 6) % 7;
  return Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate() - daysSinceMonday) - IST_OFFSET_MS;
}

// Deterministic weekly tab label: "Week of 2026-08-10" (the Monday of the week, IST).
...
export function formatIstHhMm(ms: number): string {
  const d = new Date(ms + IST_OFFSET_MS);
  return `${String(d.getUTCHours()).padStart(2, '0')}:${String(d.getUTCMinutes()).padStart(2, '0')}`;
}

// Parse "HH:MM" to milliseconds-of-day for an IST wall-clock time.
