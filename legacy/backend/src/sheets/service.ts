...
import { describeUtcInstant, formatTabName, weekMondayIst } from './time.js';
...
export class WeeklySheetService {
  ...
  preview(nowMs: number = Date.now()): Promise<WeeklyGrid> {
    return buildWeeklyGrid(this.db, weekMondayIst(nowMs));
  }
  ...
  async run(nowMs: number = Date.now()): Promise<SheetJobResult> {
    const weekMonday = weekMondayIst(nowMs);
