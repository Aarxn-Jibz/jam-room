...
export class SlotConfigService {
  ...
  async createSlot(data: SlotConfigCreateInput): Promise<SlotConfigDTO> {
    ...
    await this.assertNoOverlap(policyId, startTime, endTime);
  ...
  async updateSlot(id: string, data: SlotConfigUpdateInput): Promise<{ message: string }> {
    ...
    const nextStart = data.start_time ?? oldStart;
    const nextEnd = data.end_time ?? oldEnd;
    if (nextStart >= nextEnd) throw new Error('BAD_REQUEST: End time must be after start time');
    if (nextStart !== oldStart || nextEnd !== oldEnd) {
      await this.assertNoOverlap(policyId, nextStart, nextEnd, `${oldStart}|${oldEnd}`);
    }
  ...
  private async assertNoOverlap(policyId: string, startTime: string, endTime: string, exclude?: string): Promise<void> {
    if (startTime >= endTime) throw new Error('BAD_REQUEST: End time must be after start time');
    const schedules = await this.db.select().from(schema.operatingSchedules).where(eq(schema.operatingSchedules.policyId, policyId));
    const conflicts = schedules.some((row) => {
      if (exclude === `${row.startTime}|${row.endTime}`) return false;
      return row.startTime < endTime && row.endTime > startTime;
    });
    if (conflicts) throw new Error('BAD_REQUEST: Slot overlaps an existing configuration');
  }
}
