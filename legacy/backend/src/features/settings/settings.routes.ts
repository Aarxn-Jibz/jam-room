...
settingsAuthRoutes.use('*', requireAuth(), requirePasswordChanged(), requireAdmin());
...
settingsAuthRoutes.put('/system-settings', async (c) => {
  ...
  try {
    ...
    if ('booking_release_day' in (body as object)) set.bookingReleaseDay = data.booking_release_day;
    if ('booking_release_time' in (body as object)) set.bookingReleaseTime = data.booking_release_time;
