...
export class RequestsService {
  ...
  async create(actor: User, input: CreateRequestInput): Promise<RequestDTO> {
    ...
    if (!(await this.isReleasedFor(startTime, now))) {
      throw new Error('BAD_REQUEST: Booking has not been released yet');
    }
  ...
  async update(
    ...
    if (!isAdmin) {
      if (actor.id !== booking.userId) throw new Error('FORBIDDEN');
      const restrictedFields = ['status', 'user_id', 'band_id', 'room_id', 'slot_start', 'slot_end', 'slot_id', 'request_date', 'response_date'] as const;
      if (restrictedFields.some((field) => data[field] !== undefined)) {
        throw new Error('FORBIDDEN: Users may only edit their request reason');
      }
    }
    ...
    if (data.band_id !== undefined) {
      const profile = await this.db.select().from(schema.profiles).where(eq(schema.profiles.id, data.band_id)).limit(1);
      if (!profile[0] || !profile[0].active) throw new Error('BAD_REQUEST: Band does not exist');
    }
    ...
    try {
      ...
      } else if ((slotChanging || data.room_id !== undefined) && newStatus !== 'REJECTED') {
  ...
  async remove(actor: User, id: string): Promise<{ message: string }> {
    ...
    if (actor.role !== 'ADMIN' && existing[0].status !== 'PENDING') {
      throw new Error('FORBIDDEN: Only pending requests may be deleted');
    }
  ...
  /** Returns whether the weekly IST release moment for this booking week has passed. */
  private async isReleasedFor(startTime: number, now: number): Promise<boolean> {
    const settings = await this.db.select().from(schema.systemSettings).limit(1);
    const setting = settings[0];
    if (!setting) return true;
    const IST_OFFSET_MS = 5.5 * 60 * 60 * 1000;
    const startIst = new Date(startTime + IST_OFFSET_MS);
    const daysSinceMonday = (startIst.getUTCDay() + 6) % 7;
    const mondayUtcLike = Date.UTC(startIst.getUTCFullYear(), startIst.getUTCMonth(), startIst.getUTCDate() - daysSinceMonday);
    const daysFromReleaseToMonday = (1 - setting.bookingReleaseDay + 7) % 7;
    const [hours, minutes] = setting.bookingReleaseTime.split(':').map(Number);
    const releaseAt = mondayUtcLike - daysFromReleaseToMonday * 24 * 60 * 60 * 1000 + (hours * 60 + minutes) * 60 * 1000 - IST_OFFSET_MS;
