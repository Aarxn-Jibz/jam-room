...
describe('RequestsService', () => {
  ...
  const monday10 = Date.UTC(2026, 7, 17, 4, 30, 0); // 10:00 IST
  ...
  it('creates a pending request for an exact configured slot (10:00-11:30 normalizes to 11:00)', async () => {
    ...
    expect(request.slot_end).toBe('2026-08-17T05:30:00.000Z');
  ...
  it('rejects a booking before the configured weekly release moment', async () => {
    vi.setSystemTime(new Date('2026-08-17T03:00:00.000Z')); // 08:30 IST, before Monday 09:00 release
    await expect(service.create(member, createInput(monday10))).rejects.toThrow('Booking has not been released yet');
  });
  ...
  it('prevents an owner from changing their requested room or slot', async () => {
    const request = await service.create(member, createInput(monday10));
    await expect(service.update(member, request.id, { room_id: 'other-room' }, false)).rejects.toThrow('FORBIDDEN');
    await expect(service.update(member, request.id, { slot_start: new Date(monday10 + 60 * 60 * 1000).toISOString() }, false)).rejects.toThrow('FORBIDDEN');
  });
  ...
  it('rejects approving a booking moved into a slot occupied by another active booking', async () => {
    ...
    expect(second.slot_start).toBe('2026-08-17T05:30:00.000Z');
