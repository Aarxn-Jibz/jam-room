...
import { AppEnv, requireAdmin, requireAuth, requirePasswordChanged } from '../../middleware/auth.middleware.js';
import { WeeklySheetService } from '../../sheets/service.js';

const sheetsRoutes = new Hono<AppEnv>();
const sheetsAuthRoutes = new Hono<AppEnv>();

sheetsAuthRoutes.use('*', requireAuth(), requirePasswordChanged(), requireAdmin());
