...
function productionAdmin() {
  ...
  if (!username || !email || !password) {
    throw new Error('Remote seeding requires INITIAL_ADMIN_USERNAME, INITIAL_ADMIN_EMAIL, and INITIAL_ADMIN_PASSWORD');
  }
  if (username.length < 3 || email.length < 3 || password.length < 12) {
    throw new Error('Initial admin username/email must be valid and password must be at least 12 characters');
  }
...
VALUES ('${ADMIN_ID}', '${sqlString(admin.username)}', '${sqlString(admin.email)}', 'Admin', '${passwordHash}', 'ADMIN', 0, 1, ${now}, ${now});
...
`;
  ...
  try {
    ...
    execFileSync(
      'npx',
      ['wrangler', 'd1', 'execute', 'DB', process.argv.includes('--remote') ? '--remote' : '--local', '--file=temp_seed.sql'],
      { stdio: 'inherit' },
    );
