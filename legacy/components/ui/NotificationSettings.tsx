...
const NotificationSettings: React.FC = () => {
  ...
  const [savedReleaseDay, setSavedReleaseDay] = useState(0);
  const [savedReleaseTime, setSavedReleaseTime] = useState("13:30");
  ...
  const fetchSettings = useCallback(async () => {
    ...
    try {
      ...
      setReleaseDay(res.data.booking_release_day);
      setReleaseTime(res.data.booking_release_time);
      setSavedReleaseDay(res.data.booking_release_day);
      setSavedReleaseTime(res.data.booking_release_time);
    } catch (error) {
      console.error(error);
  ...
  const save = async () => {
    ...
    try {
      ...
      setSavedReleaseDay(releaseDay);
      setSavedReleaseTime(releaseTime);
  ...
  const dirty = email.trim() !== savedEmail || releaseDay !== savedReleaseDay || releaseTime !== savedReleaseTime;
      ...
      {loading ? (
        ...
        <div className="mt-5 pt-5 border-t border-white/10">
          <p className="text-sm text-gray-400 mb-3 font-mono">
            Release bookings weekly in Asia/Kolkata time. Requests for a week are accepted only after its release moment.
          </p>
          <div className="flex flex-col sm:flex-row gap-3">
            <select
              value={releaseDay}
              onChange={(e) => setReleaseDay(Number(e.target.value))}
              className="flex-1 font-mono px-4 py-3 bg-black-100/60 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50"
            >
              {DAYS.map((day, index) => <option key={day} value={index}>{day}</option>)}
            </select>
            <div className="flex-1"><TimePicker value={releaseTime} onChange={setReleaseTime} placeholder="Release time" /></div>
          </div>
        </div>
        </>
      )}
