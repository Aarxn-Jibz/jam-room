...
interface TimeSlot { key: string; endKey: string; display: string; end: string; }
...
// Utility functions
const istParts = (date: Date) => Object.fromEntries(
...
const istWallTimeToUtc = (day: string, time: string): Date => {
  const [year, month, date] = day.split("-").map(Number);
  const [hours, minutes] = time.split(":").map(Number);
  return new Date(Date.UTC(year, month - 1, date, hours, minutes) - 5.5 * 60 * 60 * 1000);
};
...
const RBTable = () => {
  ...
  const selectableProfiles = useMemo(() => {
    if (isAdmin) return bands;
    const assigned = session?.user.bands ?? [];
    return assigned.map(profile => {
      const details = bands.find(band => band.id === profile.id);
      return { id: profile.id, name: profile.name, colour: details?.colour ?? "#4F46E5" };
    });
  }, [bands, isAdmin, session?.user.bands]);
      ...
      try {
        ...
        const formattedConfigs: TimeSlot[] = configs.map((config) => {
          ...
          return {
            ...
            endKey: config.end_time.substring(0, 5),
  ...
  useEffect(() => {
    if (session?.user && !isAdmin) {
        setSelectedBandId((session.user as { band_id?: string }).band_id || "");
    }
  }, [session, isAdmin]);
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      ...
      try {
        ...
        fetchedSlots.forEach(slot => {
            ...
            const timeKey = formatIstTime(slotStart);
    ...
    fetchSlots();
  }, [currentWeekStart, selectedRoomNumber, dataReady, timeSlots]);
  ...
  const handleBookingConfirm = async () => {
    ...
    const slotStart = istWallTimeToUtc(day, modalTime);
    const selectedSlot = timeSlots.find((slot) => slot.key === modalTime);
    if (!selectedSlot) return;
    const slotEnd = istWallTimeToUtc(day, selectedSlot.endKey);
    ...
    try {
        ...
        const message = axios.isAxiosError(error)
          ...
          : undefined;
        setModalContent(<p className="text-red-400">{message || "An unexpected error occurred."}</p>);
    } finally {
    ...
    days.forEach(day => {
      ...
      while (i < timeSlots.length) {
        ...
        const slot = slots.find(s => {
          ...
          const tKey = formatIstTime(start);
        ...
        <Modal isOpen={isModalOpen} onClose={() => { setModalOpen(false); setShowTimeDropdown(false); }} title={modalTitle}>
            { selectedCell && modalTitle === "Request Slot" ? (
                <div className="space-y-4">
                    ...
                    {isAdmin || selectableProfiles.length > 1 ? (
                        <div className="space-y-2">
                             <label className="text-xs font-bold text-gray-400 uppercase font-mono">Select Profile</label>
                             <ProfileDropdown value={selectedBandId} onChange={setSelectedBandId} bands={selectableProfiles} />
                         </div>
                    ) : null}
                    <div className="space-y-2">
