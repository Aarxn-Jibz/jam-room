...
const UserForm = ({ user, bands, onFinished }: { user?: DbUser, bands: Band[], onFinished: () => void }) => {
  ...
  const [isAdmin, setIsAdmin] = useState(user?.role === "admin" || user?.role === "ADMIN");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    setError(null);
    const payload = { name, email, bandIds: selectedBandIds, ...(user ? {} : { role: isAdmin ? "admin" : "user" }) };
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(response?.message || response?.error || `Unable to save user (${res.status})`);
      }
      onFinished();
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {!user && (
        <label className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm font-mono text-gray-300 cursor-pointer">
          <input type="checkbox" checked={isAdmin} onChange={event => setIsAdmin(event.target.checked)} className="h-4 w-4 accent-purple-500" />
          Grant administrator access
        </label>
      )}
      {error && <p role="alert" className="rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm font-mono text-red-300">{error}</p>}
      <button type="submit" disabled={saving} className={`${btnClass} disabled:cursor-not-allowed disabled:opacity-60`}>{saving ? "Saving…" : "Save"}</button>
    </form>
  );
};
