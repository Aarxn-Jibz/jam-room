...
requestRoutes.get('/requests', async c => {
  ...
  const rows = await db.select({ id: schema.bookings.id, user_id: schema.bookings.userId, room_id: schema.bookings.roomId, band_id: schema.bookings.profileId, status: schema.bookings.status, slot_start: schema.bookings.startTime, slot_end: schema.bookings.endTime, request_date: schema.bookings.createdAt, response_date: schema.bookings.approvedAt, reason: schema.bookings.reason, user_name: schema.users.name, band_name: schema.profiles.name }).from(schema.bookings).innerJoin(schema.users, eq(schema.bookings.userId, schema.users.id)).innerJoin(schema.profiles, eq(schema.bookings.profileId, schema.profiles.id)).where(conditions.length ? and(...conditions) : undefined).orderBy(desc(schema.bookings.createdAt))
...
requestRoutes.post('/requests', async c => {
  ...
  const policy = room.policyId ? (await db.select().from(schema.bookingPolicies).where(eq(schema.bookingPolicies.id, room.policyId)).limit(1))[0] : undefined
  if (policy && start > Date.now() + policy.bookingHorizonDays * 86_400_000) return c.json({ message: 'Booking date exceeds the maximum booking horizon' }, 400)
  ...
  const configured = (await db.select().from(schema.operatingSchedules).where(and(eq(schema.operatingSchedules.enabled, true), policy ? eq(schema.operatingSchedules.policyId, policy.id) : undefined))).some(slot => slot.dayOfWeek === weekday && slot.startTime.slice(0, 5) === startTime && end - start === (Date.parse(`1970-01-01T${slot.endTime}:00Z`) - Date.parse(`1970-01-01T${slot.startTime}:00Z`)))
